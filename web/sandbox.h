...
void setup_melee(state& st, load_data_file_F&& load_data_file, int my_player, race_t my_race) {
	...
	game_load.load_map(std::forward<load_data_file_F>(load_data_file), [&]() {
		...
		game_load.setup_info.starting_minerals = 2000;  // sandbox: build/train right away
		for (int i = 0; i != 12; ++i) {
			if (i == my_player) {
...
struct play_ui : ui_functions {
	...
	enum targ_t { T_ATTACK, T_MOVE, T_PATROL, T_GATHER, T_REPAIR };
	targ_t pending_targ = T_ATTACK;

	enum cmd_act { C_MOVE, C_STOP, C_ATTACK, C_GATHER, C_HOLD, C_PATROL,
	               C_BUILDMENU, C_BUILD, C_TRAIN, C_STIM, C_SIEGE, C_UNSIEGE, C_REPAIR };
	struct cmd_t { char key; const char* label; cmd_act act; UnitTypes ut; bool enabled; };
	a_vector<cmd_t> card;
	...
	// Rebuild the context command card for the current selection (Terran).
	void refresh_card() {
		card.clear();
		card_text.clear();
		unit_t* u = primary_selected();
		...
		if (menu == 1) {
		...
		} else if (id == U::Terran_SCV) {
			title = "SCV";
			card.push_back({'m', "Move", C_MOVE, U::None});
			card.push_back({'s', "Stop", C_STOP, U::None});
			card.push_back({'a', "Attack", C_ATTACK, U::None});
			card.push_back({'g', "Gather", C_GATHER, U::None});
			card.push_back({'p', "Patrol", C_PATROL, U::None});
			card.push_back({'r', "Repair", C_REPAIR, U::None});
			card.push_back({'b', "Build", C_BUILDMENU, U::None});
		} else if (id == U::Terran_Command_Center) {
			title = "Command Center";
			card.push_back({'s', "Train SCV", C_TRAIN, U::Terran_SCV});
		} else if (id == U::Terran_Barracks) {
			title = "Barracks";
			...
			card.push_back({'g', "Ghost", C_TRAIN, U::Terran_Ghost});
		} else if (id == U::Terran_Factory) {
			title = "Factory";
			card.push_back({'v', "Vulture", C_TRAIN, U::Terran_Vulture});
			card.push_back({'t', "Siege Tank", C_TRAIN, U::Terran_Siege_Tank_Tank_Mode});
			card.push_back({'g', "Goliath", C_TRAIN, U::Terran_Goliath});
		} else if (id == U::Terran_Marine) {
			title = "Marine";
			add_move_orders();
		...
		} else if (ut_building(u)) {
			title = "Building";
		} else {
			add_move_orders();
		}
		// Gray out commands whose requirements aren't met (e.g. Ghost w/o Academy,
		// Stim before the upgrade is researched).
		for (auto& c : card) {
			switch (c.act) {
			case C_TRAIN: c.enabled = unit_can_build(u, get_unit_type(c.ut)); break;
	...
	const char* build_status() {
		...
		if (u && !u->build_queue.empty()) {
			...
			for (auto* ut : u->build_queue) { status_text += '\t'; status_text += unit_short_name(ut->id); }
		}
	...
	// Current pointer mode for the host to pick a cursor:
	// 0 normal · 1 targeting an order · 2 placing a building.
	int cursor() const { return pending_build ? 2 : targeting ? 1 : 0; }
	...
	void draw_callback(uint8_t* data, size_t data_pitch) override {
		...
		int tx = (map_pos.x - w / 2) / 32; if (tx < 0) tx = 0;
		int ty = (map_pos.y - h / 2) / 32; if (ty < 0) ty = 0;
